...
// MaxPool plugin. Receives its upstream activation stream via constructor injection
// rather than host[ActivationSource], allowing multiple pool layers in one design.
case class MaxPoolPlugin(
  cfg:      MaxPoolCore.Config,
  ...
  val logic = during build new Area {
    ...
    val outStream = BuildHelper.buildBlock(
      ...
      val core = MaxPoolCore.build(cfg, pulledStream)
